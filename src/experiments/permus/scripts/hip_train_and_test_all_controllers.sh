...
for PROBLEM_TYPE in "qap" "tsp" "lop" "pfsp"; do
    ...
    ID=`sbatch --export=PROBLEM_TYPE=$PROBLEM_TYPE,INSTANCES=$INSTANCES,MAX_TIME_PSO=0.25,N_EVALS=30,SAMPLE_SIZE_UPDATE_BK=9000,N_REEVALS_TOP_5_PERCENT=300,DESTINATION_FOLDER=src/experiments/permus/results/4by4_permu_problems/all_controllers --array=0-$NUM_INSTANCES src/experiments/permus/scripts/hip_train_array.sl`
...
NUM_INSTANCES=$( ls -1 $INSTANCES | wc -l)
NUM_INSTANCES=$(($NUM_INSTANCES - 1))
ID=`sbatch --export=PROBLEM_TYPE="qap",INSTANCES=$INSTANCES,MAX_TIME_PSO=0.25,N_EVALS=30,SAMPLE_SIZE_UPDATE_BK=9000,N_REEVALS_TOP_5_PERCENT=300,DESTINATION_FOLDER=src/experiments/permus/results/transfer_qap_with_cut_instances/all_controllers --array=0-$NUM_INSTANCES src/experiments/permus/scripts/hip_train_array.sl`
ID_2=${ID##* }
