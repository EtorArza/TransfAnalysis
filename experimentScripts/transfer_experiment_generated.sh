...
CONTROLLER_ARRAY=$(to_list "${CONTROLLER_NAME_PREFIX_ARRAY[@]}")
SEED_ARRAY=$(to_list "${SEED_ARRAY[@]}")
NLO_ARRAY=$(to_list "${NLO_ARRAY[@]}")

TESTING_JOB_ID=`sbatch --parsable --dependency=afterok:${TRAINING_JOB_ID} --export=EXPERIMENT_FOLDER_NAME=${EXPEIMENT_FOLDER_NAME},COMPUTE_RESPONSE=${COMPUTE_RESPONSE},SCORE_PATH=${SCORE_PATH},RESPONSE_PATH=${RESPONSE_PATH},TMP_RES_PATH=${TMP_RES_PATH},N_REPS=${N_REPS},N_EVALS=${N_EVALS},NEAT_POPSIZE=$NEAT_POPSIZE,CONTROLLER_ARRAY=$CONTROLLER_ARRAY,SEED_ARRAY=$SEED_ARRAY,NLO_ARRAY=$NLO_ARRAY,DIM=$DIM,SOLVER_POPSIZE=$SOLVER_POPSIZE,MAX_SOLVER_FE=$MAX_SOLVER_FE,FULL_MODEL=$FULL_MODEL --array=0-$i src/experiments/real/scripts/hip_test_array_randomly_generated.sl`
